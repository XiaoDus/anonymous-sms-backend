<template>
	<view>
		<view class="wMail">留言</view>
		<view class="uni-flex uni-column email">

			<!-- 手机号 -->
			<li class="flex-item flex-item-V phone">
				<input class="uni-input" :value="forName" placeholder="请输入TA的名字或暗号!"
					@input="forNameChange" />
			...
			<li class="flex-item flex-item-V mailText">
				<textarea class="textarea" placeholder="点击输入留言内容(表白、祝福、吐槽等...)" v-model="mailText"
					@input="inputText"></textarea>

				<text class="wordNumber">{{words}}/∞</text>
				<view>
					<text class="wordMoney">留言板不限字数，费用一共1元</text>
				</view>
			</li>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//输入手机号，出现删除符号
			forNameChange: function(event) {
				this.forName = event.detail.value;
			...
			switch1Change(e) {
				...
				if (e.detail.value) {
					this.sendText = '公开'
					this.img = uni.getStorageSync('avatarUrl')
					this.userName=uni.getStorageSync('nickName')
				} else {
					this.sendText = '匿名'
					this.img = 'https://mp-891a44a8-f007-4426-a7a4-607ad2bc43d0.cdn.bspapp.com/email/0.png'
					this.userName='匿名用户'
				}
			...
			openClick() {
				uni.showModal({
					title: '公开说明',
					content: '开启公开展示，即同意在本程序留言板中公开展示个人头像和昵称！',
					showCancel: false,
				});
			},
			...
			sendMail() {
				//验证手机号和内容
				if (this.forName && this.mailText) {
					const userPhone = uni.getStorageSync('userPhone');
					const nickName = this.userName;
					const avatarUrl = this.img;
					const forName = this.forName
														...
														if (res.data.static == 200) {
															...
															uni.navigateBack({
																success() {
																	...
																	if(beforePage.$page.fullPath=='/pages/leave/leave'){
